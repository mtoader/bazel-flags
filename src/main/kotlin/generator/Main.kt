...
class Main : CoreCliktCommand() {
  ...
  val target by option("--target", help = "Where to emit the file").path(canBeDir = false)
  ...
  override fun run() {
    ...
    val target = "${target ?: "KnownFlags"}"

    val process = ProcessBuilder(bazel, "help", "flags-as-proto").start()

    with(PrintWriter(BufferedWriter(OutputStreamWriter(FileOutputStream(target))))) {
